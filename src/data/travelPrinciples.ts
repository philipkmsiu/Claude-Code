...
8) 國家級目的地（英國／法國／瑞士／愛爾蘭等）：必須依「旅客常走的多城路線」產出景點與 hotels，覆蓋使用者選定（或常識預設）的主要城市／區域；禁止只圍繞首都＋近郊日遊卻寫成「已涵蓋全國」。英格蘭＋蘇格蘭＋愛爾蘭重要場景常見需要 18–28 天量級，recommendedDays 要反映選城範圍。
9) 全文繁體中文；不要 Markdown。
`.trim()

/** User-message ask string — same principles, shorter. */
export const AI_RESEARCH_ASK_ZH =
  '請先自行完整調研並分析這個目的地（無需使用者再提示），再輸出：季節氣候、歷史背景、難忘之處、真實景點（每點各自不同的附近美食、手信、附近購物）、分城市住宿、必吃必喝必買、建議天數與實用 tips。禁止空泛類別句。禁止把同一句美食／手信／Outlet 文案複製到每個地標。著名 Outlet／名牌村若適合此行程，必須是獨立一日購景點；其他景點只寫附近逛街。多城行程的 hotels 必須覆蓋各過夜城市。若是國家（如英國／法國／瑞士），請依選定城市／常見多城路線涵蓋主要訪客城市，不要只做首都週邊；全國重要場景的天數要明顯長於單一城市遊。'
